...
- 候選人：**Adora Xu / Galen Lin / Hsiao Jimmy**（去重後 3 人；原始名單的第四項是 Galen Lin 重複）。
- 形式：本機 web 工具。挑一張待指派任務 → 看三人的真實負載/輪替/契合 → 轉一個**忠於權重**的轉盤 → 揭曉理由 →（確認後）寫回 Notion `owner`。
...
| 發現 | 影響 |
|---|---|
| DB 共 **4884** 筆任務 | server **必須用 filter 查詢**（按 owner / status），不能每次全撈 |
...
| 候選人 | 未完成任務 | 未完成 pts | 近 14 天新任務 | fit 標籤覆蓋 |
|---|---|---|---|---|
| Adora Xu | 7 | 87 | 10 | Fault Domain 39% |
| Galen Lin | 14 | 151 | 22 | 稀疏（3–4%） |
| Hsiao Jimmy | 35 | 341 | 47 | 稀疏（4–16%） |

→ 誠實加權下轉盤會明顯偏向 Adora（最閒、最近吃最少）。這正是工具的價值。
...
| `GET` | `/api/candidates?task_id=` | 對選定任務算三人 aggregate（負載/輪替/fit）與原始分量，供前端即時調權重。 |
...
3. **契合度** `Ft_c`（低信心輔助、小權重）：
   - `tag_sim` = `t` 的 {Fault Domain, 類型} 與 `c` 頻率表的加權重疊；`t` 無標籤或三人皆無訊號 → 中性。
   - `kw_sim` = `t.title` token 與 `c` 標題 token bag 的 Jaccard/cosine（標題永遠存在，是主訊號）。
   - `Ft_raw = α·tag_sim + (1−α)·kw_sim`（α 偏小，因標籤稀疏），再 min-max；三人接近時自然趨中性。
...
- **轉盤**：三人扇形，面積＝`p_c`（誠實）；旋轉減速命中加權隨機結果，有張力。
...
- **Stage 1 — 垂直切片**：`env` + `notion_gateway` + `profiles` + `scoring` + `GET /api/tasks` + `GET /api/candidates`；極簡 HTML 顯示三張卡的**真實**負載/輪替/fit 數字 + 一個基本轉盤；寫回**僅 dry-run（stub）**。目的：及早驗證 fit 是否真有差異訊號。
